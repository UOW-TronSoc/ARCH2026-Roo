...
The firmware uses `-1` as “unset pin”.  
You must fill in your GPIO assignments in the `.ino`.

Update these sections:
- Gimbal servo pins
- Suspension servo pins
- IBT-4 forward/reverse PWM pins (left + right)
- Encoder PWM input pins (front + back)
- I2C SDA/SCL pins (if not using defaults)

> Tip: the firmware prints warnings at boot for any `-1` pins.
...
### Steps
1. Open the `.ino`
2. Set all GPIO pins (replace `-1`)
3. Set INA226 shunt value (ohms) if needed
4. Select your ESP32 board + correct COM port
...
These are the current inbound command IDs (you can change them in code):

- **ID 1**: Gimbal servo command
- **ID 2**: Suspension servo command
- **ID 3**: Drive command (left/right)
